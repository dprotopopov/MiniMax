...
{
    public class MiniMaxSystemIO : DataGridView
    {
        #region Private fields

        private const int DimentionSize = 1; //default row and col size
        ...
        public MiniMaxSystemIO()
        {
            InitializeDataGridView(DimentionSize, DimentionSize);
        }

        public MiniMaxSystemIO(int nRows, int nCols)
        {
            InitializeDataGridView(nRows, nCols);
        }

        public MiniMaxSystemIO(Point location, int nRows, int nCols)
        {
            InitializeDataGridView(nRows, nCols);
            Location = location;
        }
        ...
        private void InitializeDataGridView(int rows, int columns)
        {
            ...
            TabIndex = 0;
            RowHeadersWidth = 60;
            //used to attach event-handlers to the events of the editing control(nice header!)
            ...
            {
                AddRow(i.ToString(CultureInfo.InvariantCulture));
            }
        ...
        private void AddFunctionRow(string header)
        {
        ...
        private void AddSymbolColumn(string header)
        {
            var acolumn = new DataGridViewTextBoxColumn
            {
                HeaderText = header,
        ...
        private void AddValueColumn(string header)
        {
            var acolumn = new DataGridViewTextBoxColumn
            {
                HeaderText = header,
        ...
        private bool CheckKey(Keys K, bool isDecimalPoint, bool isMinus)
        {
            ...
            if ((K >= Keys.NumPad0) && (K <= Keys.NumPad9)) //digit from keypad?
                return true;
            if ((K >= Keys.A) && (K <= Keys.Z)) //char from keypad?
                return true;
