...
namespace MiniMax
{
    ...
    /// </summary>
    public class BooleanBranchAndBoundSolver<T> : ISolver<T>
    {
            ...
            ref IEnumerable<T> optimalValues, ITrace trace)
        {
            ...
            // Пустой вектор равносилен всем допустимым решениям задачи
            var list = new StackListQueue<BooleanTreeItem>(new BooleanTreeItem(new BooleanVector(), minimax));
            for (int k = 0;; k++)
            {
                Debug.WriteLine(string.Join(Environment.NewLine,
                    list.Select(item => string.Join(string.Empty, item.Vector.Select(x => x ? "1" : "0")))));
                ...
                // Удаляем подмножества с заведомо недопустимыми значеними
                if (appendLineCallback != null)
                    appendLineCallback("Удаляем подмножества с заведомо недопустимыми значеними");
                Debug.WriteLine("Удаляем подмножества с заведомо недопустимыми значеними");
                Debug.WriteLine(string.Join(Environment.NewLine,
                    list.Select(
                        item => string.Join(",", item.ArgBound.Select(x => x.ToString(CultureInfo.InvariantCulture))))));
                ...
                var next = new StackListQueue<BooleanTreeItem>
                {
                    list.Select(item => new BooleanTreeItem(new BooleanVector(item.Vector) {false}, minimax)),
                    list.Select(item => new BooleanTreeItem(new BooleanVector(item.Vector) {true}, minimax))
                };
                list = next;
            }
        ...
        private class BooleanTreeItem
        {
            public BooleanTreeItem(BooleanVector vector, ILinearMiniMax<T> minimax)
            {
                Vector = vector;
                double value =
